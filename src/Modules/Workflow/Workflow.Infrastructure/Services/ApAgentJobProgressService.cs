...
public sealed class ApAgentJobProgressService : IApAgentJobProgressService
{
    ...
    public async Task<ApAgentJobProgressRow?> GetByJobIdAsync(string jobId, CancellationToken cancellationToken = default)
    {
        await EnsureTableAsync(cancellationToken);
        const string sql = $"""
            SELECT TOP 1 JobId, TenantId, WorkflowId, InstanceId, HangfireState, Stage, Message, ProgressPercent, ErrorMessage, UpdatedAtUtc
            FROM {TableName}
            WHERE JobId = @JobId;
            """;
    ...
    private async Task EnsureTableAsync(CancellationToken cancellationToken)
    {
        ...
        await using var connection = OpenConnection();
        foreach (var batch in new[] { ensureSchemaSql, createTableSql, createIndexSql })
        {
