...
/// <summary>
/// Ensures DMS schema exists in tenant DB before DMS operations.
/// Applies schema on first DMS request per tenant (cached). Runs after TenantConnectionMiddleware.
/// </summary>
public sealed class DmsSchemaEnsuringMiddleware
{
    private static readonly System.Collections.Concurrent.ConcurrentDictionary<Guid, byte> AppliedTenants = new();
    private readonly RequestDelegate _next;

    public DmsSchemaEnsuringMiddleware(RequestDelegate next) => _next = next;

    public async Task InvokeAsync(
        HttpContext context,
        ITenantProvider tenantProvider,
        ITenantConnectionProvider connectionProvider,
        IDmsSchemaService schemaService)
    {
        ...
        {
            await TenantSchemaEnsureHelper.EnsureDmsSchemaAsync(
                tenantId.Value,
                conn,
