-- =============================================
-- Add [IsDefaultRepository] column to [repository].[Repositories]
...
--
-- IsDefaultRepository: 1 = default/system repo (UI read-only), 0 = user-editable repo
-- Existing rows default to 1 when column is added
-- =============================================
...
IF NOT EXISTS (
    ...
    WHERE s.name = N''repository'' AND t.name = N''Repositories'')
BEGIN
    RAISERROR(N''Table [repository].[Repositories] does not exist in this tenant database.'', 16, 1);
END

IF COL_LENGTH(N''' + REPLACE(@DbName, N']', N']]') + N'.repository.Repositories'', N''IsDefaultRepository'') IS NULL
BEGIN
    ALTER TABLE [' + REPLACE(@DbName, N']', N']]') + N'].[repository].[Repositories]
        ADD [IsDefaultRepository] BIT NOT NULL
            CONSTRAINT [DF_Repositories_IsDefaultRepository] DEFAULT (1);
END';

        EXEC sp_executesql @Sql;
        SET @Applied += 1;
        PRINT CONCAT(N'✓ OK    ', @TenantName, N' — [', @DbName, N'].[repository].[Repositories].[IsDefaultRepository]');
    END TRY
    BEGIN CATCH
        SET @Failed += 1;
        PRINT CONCAT(
